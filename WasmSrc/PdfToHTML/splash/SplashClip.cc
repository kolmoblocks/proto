...
SplashClip::SplashClip(SplashClip *clip) {
  ...
  length = clip->length;
  size = clip->size;
  paths = (SplashXPath **)malloc(size * sizeof(SplashXPath *));
  flags = (Guchar *)malloc(size * sizeof(Guchar));
  scanners = (SplashXPathScanner **)
                 malloc(size * sizeof(SplashXPathScanner *));
  for (i = 0; i < length; ++i) {
...
void SplashClip::grow(int nPaths) {
  if (length + nPaths > size) {
    ...
    paths = (SplashXPath **)realloc(paths, size * sizeof(SplashXPath *));
    flags = (Guchar *)realloc(flags, size * sizeof(Guchar));
    scanners = (SplashXPathScanner **)
                   realloc(scanners, size * sizeof(SplashXPathScanner *));
  }
}
