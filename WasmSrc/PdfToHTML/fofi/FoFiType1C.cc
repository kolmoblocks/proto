...
Gushort *FoFiType1C::getCIDToGIDMap(int *nCIDs) {
  ...
  ++n;
  map = (Gushort *)malloc(n * sizeof(Gushort));
...
void FoFiType1C::convertToCIDType0(char *psName,
  ...
  cidMap = (int *)malloc(nCIDs * sizeof(int));
  for (i = 0; i < nCIDs; ++i) {
    cidMap[i] = -1;
  }
  for (i = 0; i < nGlyphs; ++i) {
  ...
  charStringOffsets = (int *)malloc((nCIDs + 1) * sizeof(int));
  for (i = 0; i < nCIDs; ++i) {
  ...
  if (topDict.firstOp == 0x0c1e) {
	...
	return gFalse;
      ...
      privateDicts = (Type1CPrivateDict *)
	                 malloc(nFDs * sizeof(Type1CPrivateDict));
      for (i = 0; i < nFDs; ++i) {
...
void FoFiType1C::readFDSelect() {
  ...
  fdSelect = (Guchar *)malloc(nGlyphs);
...
GBool FoFiType1C::readCharset() {
  ...
  if (topDict.charsetOffset == 0) {
    ...
    charset = (Gushort *)malloc(nGlyphs * sizeof(Gushort));
    for (i = 0; i < nGlyphs; ++i) {
      charset[i] = 0;
    }
