...
void FoFiTrueType::writeTTF(FoFiOutputFunc outputFunc,
  ...
  locaTable = (TrueTypeLoca *)malloc((nGlyphs + 1) * sizeof(TrueTypeLoca));
  unsortedLoca = gFalse;
  i = seekTable("loca");
  pos = tables[i].offset;
  ok = gTrue;
  ...
  nNewTables = nTables - nZeroLengthTables +
               ...
               (missingPost ? 1 : 0);
  newTables = (TrueTypeTable *)malloc(nNewTables * sizeof(TrueTypeTable));
  j = 0;
  for (i = 0; i < nTables; ++i) {
    if (tables[i].len > 0) {
...
void FoFiTrueType::cvtSfnts(FoFiOutputFunc outputFunc,
  ...
  locaData = (Guchar *)malloc((nGlyphs + 1) * (locaFmt ? 4 : 2));
  for (i = 0; i <= nGlyphs; ++i) {
...
void FoFiTrueType::parse() {
  ...
  tables = (TrueTypeTable *)malloc(nTables * sizeof(TrueTypeTable));
  pos = 12;
  for (i = 0; i < nTables; ++i) {
  ...
  if ((i = seekTable("cmap")) >= 0) {
    ...
    cmaps = (TrueTypeCmap *)malloc(nCmaps * sizeof(TrueTypeCmap));
    for (j = 0; j < nCmaps; ++j) {
