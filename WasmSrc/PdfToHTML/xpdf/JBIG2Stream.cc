...
void JBIG2Bitmap::expand(int newH, Guint pixel) {
  ...
  data = (Guchar *)realloc(data, newH * line);
  if (pixel) {
...
{
  size = sizeA;
  bitmaps = (JBIG2Bitmap **)malloc(size * sizeof(JBIG2Bitmap *));
...
void JBIG2Stream::readSegments() {
  ...
  while (readULong(&segNum)) {
    ...
    refSegs = (Guint *)malloc(nRefSegs * sizeof(Guint));
    if (segNum <= 256) {
...
void JBIG2Stream::readSymbolDictSeg(Guint segNum, Guint length,
  ...
  bitmaps = (JBIG2Bitmap **)malloc((numInputSyms + numNewSyms) *
				    sizeof(JBIG2Bitmap *));
  k = 0;
  ...
  symWidths = NULL;
  if (huff && !refAgg) {
    symWidths = (Guint *)malloc(numNewSyms * sizeof(Guint));
  }
...
void JBIG2Stream::readTextRegionSeg(Guint segNum, GBool imm,
  ...
  syms = (JBIG2Bitmap **)malloc(numSyms * sizeof(JBIG2Bitmap *));
  kk = 0;
  for (i = 0; i < nRefSegs; ++i) {
    seg = findSegment(refSegs[i]);
    if (seg->getType() == jbig2SegSymbolDict) {
  ...
  if (huff) {
    ...
    symCodeTab = (JBIG2HuffmanTable *)malloc((numSyms + 1) *
...
void JBIG2Stream::readHalftoneRegionSeg(Guint segNum, GBool imm,
  ...
  grayImg = (Guint *)malloc(gridW * gridH * sizeof(Guint));
...
JBIG2Bitmap *JBIG2Stream::readGenericBitmap(GBool mmr, int w, int h,
  ...
  if (mmr) {
    ...
    mmrDecoder->reset();
    refLine = (int *)malloc((w + 2) * sizeof(int));
    codingLine = (int *)malloc((w + 2) * sizeof(int));
...
void JBIG2Stream::readCodeTableSeg(Guint segNum, Guint length) {
  ...
  huffTab = (JBIG2HuffmanTable *)
                malloc(huffTabSize * sizeof(JBIG2HuffmanTable));
  i = 0;
  val = lowVal;
  while (val < highVal) {
    if (i == huffTabSize) {
      huffTabSize *= 2;
      huffTab = (JBIG2HuffmanTable *)
	            realloc(huffTab, huffTabSize * sizeof(JBIG2HuffmanTable));
    }
    huffTab[i].val = val;
