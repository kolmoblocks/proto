...
void PSOutputDev::init(PSOutputFunc outputFuncA, void *outputStreamA,
  ...
  fontIDSize = 64;
  fontIDLen = 0;
  fontIDs = (Ref *)malloc(fontIDSize * sizeof(Ref));
  fontFileIDSize = 64;
  fontFileIDLen = 0;
  fontFileIDs = (Ref *)malloc(fontFileIDSize * sizeof(Ref));
  fontFileNameSize = 64;
  fontFileNameLen = 0;
  fontFileNames = (GString **)malloc(fontFileNameSize * sizeof(GString *));
  nextTrueTypeNum = 0;
  font16EncLen = 0;
  font16EncSize = 0;
...
void PSOutputDev::setupFont(GfxFont *font, Dict *parentResDict) {
  ...
  if (fontIDLen >= fontIDSize) {
    fontIDSize += 64;
    fontIDs = (Ref *)realloc(fontIDs, fontIDSize * sizeof(Ref));
  }
  ...
  // do 16-bit font substitution
    ...
    if (font16EncLen >= font16EncSize) {
      font16EncSize += 16;
      font16Enc = (PSFont16Enc *)realloc(font16Enc,
...
void PSOutputDev::setupEmbeddedType1Font(Ref *id, GString *psName) {
  ...
  if (fontFileIDLen >= fontFileIDSize) {
    fontFileIDSize += 64;
    fontFileIDs = (Ref *)realloc(fontFileIDs, fontFileIDSize * sizeof(Ref));
  }
...
void PSOutputDev::setupExternalType1Font(GString *fileName, GString *psName) {
  ...
  if (fontFileNameLen >= fontFileNameSize) {
    fontFileNameSize += 64;
    fontFileNames = (GString **)realloc(fontFileNames,
