...
GBool JPXStream::readBoxes() {
  ...
  if (str->lookChar() == 0xff) {
    ...
    readCodestream(0);
    nComps = img.nComps;
    bpc = (Guint *)malloc(nComps * sizeof(Guint));
    for (i = 0; i < nComps; ++i) {
  ...
  while (readBoxHdr(&boxType, &boxLen, &dataLen)) {
	...
	return gFalse;
      ...
      palette.bpc = (Guint *)malloc(palette.nComps * sizeof(Guint));
      palette.c =
          (int *)malloc(palette.nEntries * palette.nComps * sizeof(int));
    ...
    case 0x636d6170:		// component mapping
      ...
      compMap.comp = (Guint *)malloc(compMap.nChannels * sizeof(Guint));
      compMap.type = (Guint *)malloc(compMap.nChannels * sizeof(Guint));
      compMap.pComp = (Guint *)malloc(compMap.nChannels * sizeof(Guint));
	...
	return gFalse;
      ...
      channelDefn.idx =
	  (Guint *)malloc(channelDefn.nChannels * sizeof(Guint));
      channelDefn.type =
	  (Guint *)malloc(channelDefn.nChannels * sizeof(Guint));
      channelDefn.assoc =
	  (Guint *)malloc(channelDefn.nChannels * sizeof(Guint));
  ...
  do {
	...
	return gFalse;
      ...
      img.nYTiles = (img.ySize - img.yTileOffset + img.yTileSize - 1)
	            / img.yTileSize;
      img.tiles = (JPXTile *)malloc(img.nXTiles * img.nYTiles *
				     sizeof(JPXTile));
      for (i = 0; i < img.nXTiles * img.nYTiles; ++i) {
	img.tiles[i].tileComps = (JPXTileComp *)malloc(img.nComps *
							sizeof(JPXTileComp));
	for (comp = 0; comp < img.nComps; ++comp) {
	...
	img.tiles[0].tileComps[0].quantSteps =
	    (Guint *)realloc(img.tiles[0].tileComps[0].quantSteps,
	...
	for (comp = 0; comp < img.nComps; ++comp) {
	  if (!(i == 0 && comp == 0)) {
	    ...
	    img.tiles[i].tileComps[comp].quantSteps = 
	        (Guint *)realloc(img.tiles[i].tileComps[comp].quantSteps,
				  img.tiles[0].tileComps[0].nQuantSteps *
				    sizeof(Guint));
	...
	img.tiles[0].tileComps[comp].quantSteps =
	    (Guint *)realloc(img.tiles[0].tileComps[comp].quantSteps,
			      img.tiles[0].tileComps[comp].nQuantSteps *
			        sizeof(Guint));
...
GBool JPXStream::readTilePart() {
  ...
  do {
	...
	img.tiles[tileIdx].tileComps[0].quantSteps =
	    (Guint *)realloc(img.tiles[tileIdx].tileComps[0].quantSteps,
			      img.tiles[tileIdx].tileComps[0].nQuantSteps *
			        sizeof(Guint));
	...
	img.tiles[tileIdx].tileComps[comp].quantSteps = 
	    (Guint *)realloc(img.tiles[tileIdx].tileComps[comp].quantSteps,
			      img.tiles[tileIdx].tileComps[0].nQuantSteps *
			        sizeof(Guint));
  ...
  if (tilePartIdx == 0) {
	...
	resLevel->precincts = (JPXPrecinct *)malloc(1 * sizeof(JPXPrecinct));
	for (pre = 0; pre < 1; ++pre) {
	  precinct = &resLevel->precincts[pre];
	  ...
	  nSBs = r == 0 ? 1 : 3;
	  precinct->subbands =
	      (JPXSubband *)malloc(nSBs * sizeof(JPXSubband));
	  for (sb = 0; sb < nSBs; ++sb) {
	    subband = &precinct->subbands[sb];
	    ...
	    subband->inclusion =
	        (JPXTagTreeNode *)malloc(n * sizeof(JPXTagTreeNode));
	    ...
	    subband->cbs = (JPXCodeBlock *)malloc(subband->nXCBs *
						   subband->nYCBs *
						   sizeof(JPXCodeBlock));
		...
		cb->nZeroBitPlanes = 0;
		cb->coeffs =
		    (JPXCoeff *)malloc((1 << (tileComp->codeBlockW
					       + tileComp->codeBlockH))
					* sizeof(JPXCoeff));
		for (cbi = 0;
