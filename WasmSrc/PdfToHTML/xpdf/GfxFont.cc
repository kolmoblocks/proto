...
Gushort *Gfx8BitFont::getCodeToGIDMap(FoFiTrueType *ff) {
  ...
  map = (Gushort *)malloc(256 * sizeof(Gushort));
  for (i = 0; i < 256; ++i) {
    map[i] = 0;
  }
  ...
  GfxFont(tagA, idA, nameA)
{
  ...
  if (type == fontCIDType2) {
    ...
    if (obj1.isStream()) {
      cidToGIDLen = 0;
      i = 64;
      cidToGID = (Gushort *)malloc(i * sizeof(Gushort));
      obj1.streamReset();
      while ((c1 = obj1.streamGetChar()) != EOF &&
	     (c2 = obj1.streamGetChar()) != EOF) {
	if (cidToGIDLen == i) {
	  i *= 2;
	  cidToGID = (Gushort *)realloc(cidToGID, i * sizeof(Gushort));
	}
...
GfxFontDict::GfxFontDict(XRef *xref, Ref *fontDictRef, Dict *fontDict) {
  ...
  fonts = (GfxFont **)malloc(numFonts * sizeof(GfxFont *));
