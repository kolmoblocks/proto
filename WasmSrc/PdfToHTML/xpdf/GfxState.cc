...
GfxIndexedColorSpace::GfxIndexedColorSpace(GfxColorSpace *baseA,
  ...
  lookup = (Guchar *)malloc((indexHigh + 1) * base->getNComps() *
			     sizeof(Guchar));
}
...
GfxImageColorMap::GfxImageColorMap(int bitsA, Object *decode,
  ...
  if (colorSpace->getMode() == csIndexed) {
    ...
    lookup = (double *)malloc((maxPixel + 1) * nComps2 * sizeof(double));
    ...
    lookup = (double *)malloc((maxPixel + 1) * nComps * sizeof(double));
    for (i = 0; i <= maxPixel; ++i) {
      for (k = 0; k < nComps; ++k) {
...
GfxSubpath::GfxSubpath(double x1, double y1) {
  ...
  curve = (GBool *)malloc(size * sizeof(GBool));
  n = 1;
  x[0] = x1;
  y[0] = y1;
  curve[0] = gFalse;
...
void GfxSubpath::lineTo(double x1, double y1) {
  if (n >= size) {
    ...
    curve = (GBool *)realloc(curve, size * sizeof(GBool));
  }
  x[n] = x1;
  y[n] = y1;
  curve[n] = gFalse;
...
GfxPath::GfxPath() {
  ...
  n = 0;
  firstX = firstY = 0;
  subpaths = (GfxSubpath **)malloc(size * sizeof(GfxSubpath *));
}
...
void GfxPath::lineTo(double x, double y) {
  if (justMoved) {
    if (n >= size) {
      size += 16;
      subpaths = (GfxSubpath **)
	           realloc(subpaths, size * sizeof(GfxSubpath *));
    }
