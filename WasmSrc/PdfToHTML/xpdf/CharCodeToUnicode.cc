...
CharCodeToUnicode *CharCodeToUnicode::parseCIDToUnicode(GString *fileName,
  ...
  size = 32768;
  mapA = (Unicode *)malloc(size * sizeof(Unicode));
  mapLenA = 0;

  while (getLine(buf, sizeof(buf), f)) {
    if (mapLenA == size) {
      size *= 2;
      mapA = (Unicode *)realloc(mapA, size * sizeof(Unicode));
    }
    /*if (sscanf(buf, "%x", &u) == 1) {
      mapA[mapLenA] = u;
    } else {
...
CharCodeToUnicode *CharCodeToUnicode::parseUnicodeToUnicode(
  ...
  while (getLine(buf, sizeof(buf), f)) {
	...
	sMapA = (CharCodeToUnicodeString *)
	          realloc(sMapA, sMapSizeA * sizeof(CharCodeToUnicodeString));
      }
      sMapA[sMapLenA].c = u0;
      for (i = 0; i < n; ++i) {
...
void CharCodeToUnicode::addMapping(CharCode code, char *uStr, int n,
  ...
  if (code >= mapLen) {
    oldLen = mapLen;
    mapLen = (code + 256) & ~255;
    map = (Unicode *)realloc(map, mapLen * sizeof(Unicode));
  ...
  if (n <= 4) {
    ...
    if (sMapLen >= sMapSize) {
      ...
      sMap = (CharCodeToUnicodeString *)
	       realloc(sMap, sMapSize * sizeof(CharCodeToUnicodeString));
    }
    map[code] = 0;
    sMap[sMapLen].c = code;
...
CharCodeToUnicode::CharCodeToUnicode(GString *tagA) {
  ...
  tag = tagA;
  mapLen = 256;
  map = (Unicode *)malloc(mapLen * sizeof(Unicode));
  for (i = 0; i < mapLen; ++i) {
    map[i] = 0;
  }
  sMap = NULL;
...
CharCodeToUnicodeCache::CharCodeToUnicodeCache(int sizeA) {
  ...
  size = sizeA;
  cache = (CharCodeToUnicode **)malloc(size * sizeof(CharCodeToUnicode *));
  for (i = 0; i < size; ++i) {
    cache[i] = NULL;
  }
}
