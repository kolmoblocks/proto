...
ImageStream::ImageStream(Stream *strA, int widthA, int nCompsA, int nBitsA) {
  ...
  imgLine = (Guchar *)malloc(imgLineSize * sizeof(Guchar));
  imgIdx = nVals;
}
...
StreamPredictor::StreamPredictor(Stream *strA, int predictorA,
  ...
  predLine = (Guchar *)malloc(rowBytes);
...
CCITTFaxStream::CCITTFaxStream(Stream *strA, int encodingA, GBool endOfLineA,
  ...
  black = blackA;
  refLine = (short *)malloc((columns + 3) * sizeof(short));
  codingLine = (short *)malloc((columns + 2) * sizeof(short));
...
void DCTStream::reset() {
  ...
  if (progressive || !interleaved) {
    ...
    for (i = 0; i < numComps; ++i) {
      for (j = 0; j < mcuHeight; ++j) {
	rowBuf[i][j] = (Guchar *)malloc(bufWidth * sizeof(Guchar));
      }
    }
